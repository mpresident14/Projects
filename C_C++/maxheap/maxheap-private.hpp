...
void MaxHeap<T>::bubbleDown(size_t index)
{
    ...
    // If node only has left child, check if they need to be swapped
        ...
        bool left = true;
        T*& largerChild = *leftChild < *rightChild ? 
...
template <typename T>
bool MaxHeap<T>::replaceElementHelper(size_t index, const T& toDelete, const T& toInsert)
{
    T*& parent = arr_[index];
    size_t leftIndex = 2*index + 1;
    size_t rightIndex = 2*index + 2;
...
template <typename T>
ostream& MaxHeap<T>::printHelper(size_t index, std::ostream& out, size_t level) const
{
    if (index < size_){
