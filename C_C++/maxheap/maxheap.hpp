...
class MaxHeap {
 public:
  ...
  /**
   * @brief Replace all occurrences of an element in the MaxHeap with another
   * element
   * @details More efficient than deleting and then inserting because it
 ...
 private:
  static const size_t ORIGINAL_CAPACITY =
      4;  ///< Starting size of array data member
   ...
   */
  std::ostream& printHelper(size_t index, std::ostream& out,
                            size_t level) const;
};
