 ...
 public:
  ...
  static void check(bool statement, size_t line, std::string file) {
    ...
    if (line >= currentLine) {
      ...
      // Grab the affirm statement and save in hashmap in case it is needed
      // later
      getline(in, s);
      prevLines[line] = s;
      currentLine = line + 1;
    ...
    if (!statement) {
      ...
      ++fails_;
      // Print details
      std::cerr << "FAILURE: " << file << ", line " << line << ": " << s
                << std::endl;
  ...
  static void printResults() {
    std::cerr << "Result: Failed " << fails_ << " / " << totalChecks_
              << " affirmations."
              << "\n"
              << std::endl;
  }
...
#define affirm(statement) TestingProgram::check(statement, __LINE__, __FILE__)
