...
void Pegger::addToBoard(Card*& card, vector<Card*>& board,
                        size_t& runningTotal) {
  ...
  if (bestScore == 0) {
    ...
    for (int i = cardsLeft_ - 1; i >= 0; --i) {
      if (runningTotal + chosenHand_[i]->adder() <= 31 &&
        ...
        // If all criteria are met and I have a might have a pair royal chance,
        // play this card
...
bool Pegger::pairRoyalChance(size_t cardIndex, size_t& runningTotal) const {
  ...
  return cardIndex != 0 &&
         *(chosenHand_[cardIndex]) == *(chosenHand_[cardIndex - 1]) &&
         runningTotal + 3 * (chosenHand_[cardIndex])->adder() <= 31;
}

bool Pegger::startWithSub5(Card*& cardToPlay, size_t& runningTotal) const {
  if (runningTotal !=
      0) {  // Can't start with a card below 5 if not the first card
    return false;
  }
  return cardToPlay->adder() < 5;
}

bool Pegger::sub5StartNo10(Card*& cardToPlay, vector<Card*>& board,
                           size_t& runningTotal) const {
  return board.size() == 1 && runningTotal < 5 && cardToPlay->adder() == 10;
}

bool Pegger::makesRunOpportunity(Card*& cardToPlay, vector<Card*>& board,
                                 size_t& runningTotal) const {
  if (board.size() ==
      0) {  // If no cards on board, can't make a run opportunity
    return false;
  }
  ...
  if (abs(long(cardValue - lastOnBoard)) == 1) {
    if (cardValue > lastOnBoard) {
      if (runningTotal + cardValue + 1 <= 31 ||
          runningTotal + lastOnBoard - 1 <= 31) {
        return true;
      }
      return false;
    } else {
      if (runningTotal + lastOnBoard + 1 <= 31 ||
          runningTotal + cardValue - 1 <= 31) {
        return true;
      }
      return false;
    }
...
bool Pegger::hasCards() const { return cardsLeft_ != 0; }
...
size_t Pegger::scorePeg(Card*& card, vector<Card*>& board, size_t& runningTotal,
                        bool print) const {
  return pegPair(card, board, print) + peg15(card, runningTotal, print) +
         peg31(card, runningTotal, print) + pegRun(card, board, print);
}
...
size_t Pegger::pegRun(Card*& card, vector<Card*>& board, bool print) const {
  ...
  for (auto iter = board.rbegin(); iter != board.rend() && length < 8; ++iter) {
    ...
    if ((max + min) * double(length) / 2 == total && max - min + 1 == length) {
      bestRun = length;
    }
