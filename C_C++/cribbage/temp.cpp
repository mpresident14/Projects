...
Cribbage::Cribbage() : runningTotal_{0}, player1Score_{0}, player2Score_{0} {
...
void Cribbage::theGo(Card**& userChoice, Card**& compChoice, bool userHasCrib) {
  ...
  while (pegger1.hasCards() || pegger2.hasCards()) {
    ...
    if (pegger1.canPlayUnder31(runningTotal_)) {
      cout << "\n"
           << "YOUR CARDS: ";
      printCardArray(pegger1.chosenHand_,
                     pegger1.cardsLeft_);  // Show user their cards
...
Card* Cribbage::userSelectCardToPlay(Card**& hand, size_t& cardsLeft,
                                     size_t& runningTotal) {
  cout << "\n"
  ...
  while (true) {
    ...
    if (userGoesFirst) {
      ...
      size_t compInsert = Player::insertCardIntoSortedArray(
          compCopy, turnUp);  // Insert the turn up card
    ...
    if (turnUp->value() == 11) {
      player2Score_ += 2;
      cout << "Turn-up Card is a Jack. Opponent scores 2."
           << endl;  // **DIFFERENT
      if (checkForWin()) {
        break;
      }
    ...
    size_t userInsert = Player::insertCardIntoSortedArray(
        userCopy, turnUp);  // Insert the turn up card
    size_t userScore = player1.scoreOf(userCopy, 5, userInsert, false, true);
    cout << "Score: " << userScore << endl;
    player1Score_ += userScore;
    if (checkForWin()) {
    ...
    cout << "\n"
         << "OPPONENT'S CRIB: ";
    printCardArray(cribCopy, 4);
    Player::insertCardIntoSortedArray(cribCopy,
                                      turnUp);  // Insert the turn up card
