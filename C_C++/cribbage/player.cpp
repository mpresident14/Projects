...
// CONSIDER CONSTRUCTING A PLAYER WITH A HAND EACH TIME. THEN PUT THE SETUP IN THE CONSTRUCTOR.
// CREATE A DIFFERENT CLASS TO DEAL WITH PEGGING (class PeggingPlayer ???)
// GIVE PLAYERS A NAME SO THAT WE CAN DIFFERENTIATE THEM WHEN PLAYING AGAINST EACH OTHER
// IF COMPUTER DECISION TAKES A LONG TIME, HAVE ANOTHER THREAD RUNNING SO THAT ALGORITHM CAN RUN WHILE PLAYER MAKES THEIR DECISION
Player::Player(Card** hand)
    :hand_{hand}, suitCounts_{{HEARTS, 0}, {DIAMONDS, 0}, {SPADES,0}, {CLUBS,0}}
{
    ...
    // Put all cards in the deck, and then remove the cards in the hand (faster than checking if each card is in the hand)
...
{
    for (auto iter = remainingCards_.begin(); iter != remainingCards_.end(); ++iter){
...
{    
    ...
    Card** chosen = new Card* [5](); // 4 card hand in cribbage, leave 1 open for the turn-up card
    Card** crib = new Card* [5]();

    double bestScore = 0;
    split[0] = new Card* [5](); // split[0] is the best choice, leave 1 spot open for the turn-up card

    optimalDiscardHelper(chosen, crib, 6, 0, 0, bestScore, split[0], hasCrib); // 6 cards to start (2-player game)
...
void Player::optimalDiscardHelper(Card**& chosen, Card**& crib, size_t length,
    size_t chosenIndex, size_t getFromIndex, double& bestScore, Card**& bestChoice, bool hasCrib)
{
    ...
    if (chosenIndex == 4){ // 4 card hand in cribbage
        ...
        double avgHandScore = nobsPercentage(chosen, 4) + double(flipTurnUpCard(chosen)) / 46; // 46 possible turn-up cards
        double avgCribScore = nobsPercentage(crib, 2) + double(flipCardsForCrib(crib)) / 15180; // 46 choosing 3 possible combos of turnup + 2 other crib cards
        ...
        // Recursion in for-loop to get every combination of 4 cards from the 6 cards in hand
        for (size_t i = getFromIndex; i < 9-length; ++i){ // i < hand.length (6) - length + (hand.length - chosen.length + 1) (3)
            
            chosen[chosenIndex] = hand_[i];
            optimalDiscardHelper(chosen, crib, length-1, chosenIndex+1, i+1, bestScore, bestChoice, hasCrib);
        }   
    }
}
...
{    
    ...
    for (size_t i = 0; i < 46; ++i){ // 46 cards remaining
        ...
        size_t insertedIndex = insertCardIntoSortedArray(chosen, remainingCards_[i]); 
        score += scoreOf(chosen, 5, insertedIndex, false, false, false); // No need to sort, already sorted
...
{
    size_t score = 0;
    flipCardsForCribHelper(crib, 46, 2, 0, score); // deck.length == 46, starting at index 2 of crib b/c first 2 spots are the discarded cards    
    return score;
}

void Player::flipCardsForCribHelper(Card**& crib, size_t length, size_t cribIndex, size_t deckIndex, size_t& score)
{
    // We have chosen 3 more cards to give a total of 5 cards
    if (cribIndex == 5){
        Card** copy = new Card*[5];
        for (size_t i = 0; i < 5; ++i){ // Make copy so crib is not changed by sort in scoreOf
            copy[i] = crib[i];
        }

        score += scoreOf(copy, 5, 0, true, true, false); // Need to sort these, 0 b/c no turnCardIndex
        delete[] copy;
    }
    else{
        // Recursion in for-loop to get every combination of 4 cards from the 6 cards in hand
        for (size_t i = deckIndex; i < 90-length; ++i){ // i < DECK.length (46) - length + (DECK.length - crib.length + 1) (44)____(crib.length is # cards to add == 3)
...
void Player::removeInsertedCard(Card**& array, size_t insertedIndex) // Length of array is 4
{
...
double Player::nobsPercentage(Card**& array, size_t length)
{
    ...
    for (size_t i = 0; i < length; ++i){
        if ( *(array[i]) == 11){
            percentage += double(13 - suitCounts_[array[i]->suit()]) / 46; // For each jack, (13 total of the suit - number of suits in the hand)
        }                                                                  // all divided by 46 cards remaining in the deck
    }
    return percentage;
}

// Hand is sorted by Player constructor. Need to sort again if we are determining optimal discard for Crib
size_t Player::scoreOf(Card**& hand, size_t length, size_t turnCardIndex, bool inCrib, bool needsSort, bool print)
{
    if (needsSort){
    ...
    // int64_t elapsedTime = chrono::duration_cast<chrono::nanoseconds>(stop - start).count();
    // cout << elapsedTime << " seconds" << endl;
    
    // start = chrono::steady_clock::now();
    // size_t k = scoreRuns(hand, length);
    // stop = chrono::steady_clock::now();
    // elapsedTime = chrono::duration_cast<chrono::nanoseconds>(stop - start).count();
    // cout << elapsedTime << " seconds" << endl;
    ...
    return scoreFifteens(hand, print) + // Only works for an array of length 5
           scoreRuns(hand, length, print) +
           scorePairs(hand, length, print) +
...
size_t Player::scoreFifteens(Card**& hand, bool print)
{
    ...
    size_t count = findGroupsOf2Scoring15(values, 5) + findGroupsOf3Scoring15(values, 5);
...
size_t Player::scoreRuns(Card**& hand, size_t length, bool print)
{
    ...
    // Loop through and check whether the next element is one greater than the current
    for (size_t i = 0; i < length-1; ++i){
        if (*(hand[i+1]) == *(hand[i]) + 1){
            ...
            if (score >= 3){
                if (print){
                    cout << multiplier << " run(s) of " << score << " for " << score*multiplier << "." << endl;
                }
...
size_t Player::scorePairs(Card**& hand, size_t length, bool print)
{
    ...
    if (print && group1 + group2 != 0){
        cout << (group1 + group2) / 2 << " pair(s) for " << group1 + group2 << "." << endl;
    }
    return group1 + group2;

}

size_t Player::scoreFlush(Card**& hand, size_t length, size_t turnCardIndex, bool inCrib, bool print)
{
    int counts[4] {0,0,0,0}; // These are the counts of each suit, index corresponds to Enum Suit value
    
    if (inCrib){ // In crib, doesn't matter which is turn card b/c all five need to be same suit
        for (size_t i = 0; i < length; ++i){
            ++counts[hand[i]->suit()];
        }
    }
    ...
    for (size_t currentSuit = 0; currentSuit < 4; ++currentSuit){
        if (counts[currentSuit] > 3){
            ...
            if (!inCrib){
                if (size_t (hand[turnCardIndex]->suit()) == currentSuit){ // If turnCard suit is same as 4 cards in your hand, flush is worth 5
                    if (print){
                        cout << "Flush for 5." << endl;
                    }
                    return 5;
