...
vector<string> get_all_ips(const char* domain) {
  ...
  unsigned char* data =
      new unsigned char[sizeof(struct dns_header) + question_len];
  ...
  *dnsclass = htons(1);
  ...
  inet_pton(AF_INET, dns_server_ip,
            &(dns_addr.sin_addr));  // Convert DNS IP to network format
  ...
  if (sendto(sd, data, sizeof(struct dns_header) + question_len, 0,
             (const struct sockaddr*)&dns_addr, sizeof(struct sockaddr)) < 0) {
    perror("sendto");
    close(sd);
    return resolved_ips;
  }
...
// 	for (auto iter = resolved_ips->begin(); iter != resolved_ips->end();
// ++iter) { 		printf("%s\n", iter->c_str());
// 	}
