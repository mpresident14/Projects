...
class Player {
 public:
  ...
  void getHandCountsHelper(const std::vector<Card>& deck, Card* combo,
                           uchar deckSize, uchar comboSize, uchar deckIndex,
                           uchar comboIndex);
   ...
   */
  uchar value_arr_[NUMVALUES];  // Array keeping track of count of each value
                                // (0=2, 1=3,..., 11=K, 12=A)
...
inline char* dec_to_bin_nbit(uchar num, size_t n) {
  ...
  while (i < n) {
    // Store remainder bit
    bin_str[n - 1 - i] =
        (num % 2) + 48;  // +48 converts '0' and '1' to 0 and 1, respectively
    num = num / 2;
    i++;
  }
  return bin_str;
