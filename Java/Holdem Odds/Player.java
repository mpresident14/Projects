...
//-----------------------------------------------------------------------------------------------
    ...
    private OneCardHandPercentages getPercentagesArrayFromSymbol(Symbol symbol){
    	switch (symbol){
	    	case TWO:
	    		return OneCardHandPercentages.TWO;
	    	case THREE:
	    		return OneCardHandPercentages.THREE;
	    	case FOUR:
	    		return OneCardHandPercentages.FOUR;
	    	case FIVE:
	    		return OneCardHandPercentages.FIVE;
	    	case SIX:
	    		return OneCardHandPercentages.SIX;
	    	case SEVEN:
	    		return OneCardHandPercentages.SEVEN;
	    	case EIGHT:
	    		return OneCardHandPercentages.EIGHT;
	    	case NINE:
	    		return OneCardHandPercentages.NINE;
	    	case TEN:
	    		return OneCardHandPercentages.TEN;
	    	case JACK:
	    		return OneCardHandPercentages.JACK;
	    	case QUEEN:
	    		return OneCardHandPercentages.QUEEN;
	    	default:
	    		return OneCardHandPercentages.KING;
    	}
    }
    ...
    public double[] getPercentages(int numCardsToBeDealt){
    	// If zero cards (dealing 7)
    	if (numCardsToBeDealt == 7){
    		return new double[] {0.1741192, 0.4382255, 0.2349554, 0.0482987, 0.0461938, 0.0302549, 0.025961, 0.0016807, 2.785E-4, 3.23E-5};
    	}
    	
    	
    	// If only one card (dealing 6)
        if (numCardsToBeDealt == 6){
        	if (this.hand.length != 0){
        		OneCardHandPercentages percent = this.getPercentagesArrayFromSymbol(this.hand[0].symbol);
        		System.out.println(percent);
        		return percent.percentages;
        	}
        	
        	else{
        		return this.getPercentagesArrayFromSymbol(this.board[0].symbol).percentages;
        	}
        }
...
//---------------------------------------------------------------------------------------------------------------------------------------
    ...
    public static void main(String[] args) {
        Card h1 = new Card(Symbol.ACE, Suit.CLUBS);
...
//        System.out.println(c1.compareTo(c2));
        ...
        System.out.println(Arrays.toString(p1.getPercentages(7)));
