...
class SlidePuzzle {
 public:
  SlidePuzzle() = delete;
  SlidePuzzle(
      const int* grid);  // Must have exactly one -1 to signal empty position
  ...
  std::forward_list<SlidePuzzle> solvePuzzle(
      const SlidePuzzle& completed) const;

  friend inline std::ostream& operator<<(
      std::ostream& out, const SlidePuzzle<width, height>& puzzle) {
