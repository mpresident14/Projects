...
string noParseMsgType(string expected, string got) {
  return "Parse error: Expected an object of type " + expected + ", got \"" +
         got + "\".";
}
...
void testCharParser() {
  ...
  tester.assertThrowsWithMsg(
      [&any]() { any.parseAll("xx"); }, remainingMsg("x"));
  tester.assertThrowsWithMsg(
      [&any]() { any.parseAll(""); }, noParseMsgType("<char>", ""));
}
...
void testThisChar() {
  ...
  tester.assertThrows([&a]() { a.parseAll("y"); });
  tester.assertThrows([&a]() { a.parseAll("xx"); });
}
...
void testStringParser() {
  ...
  tester.assertThrows([&hello]() { hello.parseAll("parsers"); });
  tester.assertThrowsWithMsg(
      [&hello]() { hello.parseAll("parse"); },
      noParseMsg("\"parser\"", "parse"));
  tester.assertThrows([&hello]() { hello.parseAll("hello"); });
}

void testConditionalParser() {
  auto lessThan100 =
      doOnlyIf(anyULong, [](unsigned long n) { return n < 100; });
  ...
  tester.assertThrows([&lessThan100]() { lessThan100.parseAll("abc"); });
}

void testMapParser() {
  auto strLen = transform(thisString("hello"), [](string&& s) {
...
void testManyParser() {
  ...
  tester.assertTrue(
      (is_same_v<ignore_t, decltype(ignoreManyDigits.parseAll("12345"))>));
  ignoreManyDigits.parseAll("12345");
  tester.assertThrows(
      [&ignoreManyDigits]() { ignoreManyDigits.parseAll("123ab"); });

  tester.assertEquals("aaaa", manyAs.parseAll("aaaa"));
  tester.assertEquals("", manyAs.parseAll(""));
  tester.assertThrows([&manyAs]() { manyAs.parseAll("aaab"); });
  ...
  tester.assertThrows([&manyNumbers]() { manyNumbers.parseAll("100a"); });
}
...
void testAltParser() {
  ...
  tester.assertThrowsWithMsg([&abc]() { abc.parseAll("d"); }, errMsg.str());
  ...
  auto betterStrP =
      alt(thisString("antsy"), thisString("ants"), thisString("ant"));
...
void testSeqParser() {
  ...
  tester.assertEquals(
      std::tuple(string("hello"), 'a', 1234),
      hodgepodge.parseAll("helloa  1234"));
  tester.assertThrowsWithMsg(
  ...
  tester.assertEquals(
      std::tuple(string("hello"), 'b', 1234), withAlt.parseAll("hellob  1234"));

  auto withSkip = seq(skip(hello), anyULong);
  tester.assertEquals(std::tuple(1234), withSkip.parseAll("hello 1234"));
  tester.assertThrows([&withSkip]() { withSkip.parseAll("1234"); });
}
...
void testLazyParser() {
  ...
  tester.assertThrowsWithMsg(
      ...
      noParseMsgType("<unsigned long>", "+ 45") +
          " (To transform into type <unsigned long>)");
}
