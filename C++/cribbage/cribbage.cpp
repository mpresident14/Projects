...
Cribbage::Cribbage()
    : runningTotal_{0}, player1Score_{0}, player2Score_{0},goPoints1_{0}, goPoints2_{0},
      handPoints1_{0}, handPoints2_{0}, cribPoints1_{0}, cribPoints2_{0}
{
...
void Cribbage::theGo(vector<Card*>& userChoiceVec, vector<Card*>& compChoiceVec, bool userHasCrib)
{
    cout << "****************************" << endl;
    cout << "* ---------THE GO--------- *" << endl;
    cout << "****************************" << endl;
...
Card* Cribbage::userSelectCardToPlay(vector<Card*>& hand, size_t& cardsLeft, size_t& runningTotal)
{
...
void Cribbage::play()
{
    ...
    while(true){
        if (userGoesFirst){
            ...
            cout << "\n" << "OPPONENT'S CARDS: ";
            printCardArray(compChoice, 4); // Print array before inserting turn up card
            size_t compInsert = Player::insertCardIntoSortedArray(compChoice, turnUp); // Insert the turn up card
            size_t compScore = player2.scoreOf(compChoice, 5, compInsert, false, false, true);
            if (compNobs){
            ...
            size_t userScore = player1.scoreOf(userChoice, 5, userInsert, false, false, true);
            if (userNobs){
                cout << "Nobs for 1." << endl;
                ++userScore;
            }
        ...
        cout << "\n" << "YOUR CARDS: ";
        printCardArray(userChoice, 4);
        size_t userInsert = Player::insertCardIntoSortedArray(userChoice, turnUp); // Insert the turn up card
        size_t userScore = player1.scoreOf(userChoice, 5, userInsert, false, false, true);
        if (userNobs){
            cout << "Nobs for 1." << endl;
            ++userScore;
