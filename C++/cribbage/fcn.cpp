        ...
        for (size_t right = length-1; right > left; --right){
            ...
            for (size_t i = left + 1; i < right; ++i){
                ...
                for (size_t j = i+1; j < right; ++j){
                    ...
                    if (total == 15){
                        ++count;
                        break;  // j adds on to total, so no need to add anymore if we hit 15 already
                    }
                    else if (total > 15){
                        break;
                    }
...
void getPermutsHelper(size_t*& hand, size_t*& chosen, size_t length, size_t chosenIndex, size_t getFromIndex)
{
    if (chosenIndex == 4){
        printArray(chosen, 4);
    }
