...
def predict_validation_and_calculate_loss(model, num_examples):
    ...
    # Calculate loss
    loss = np.square(probs - y_validation) # using mean squared error sum( (pred_y - actual_y)^2 )
    total_loss = np.sum(loss)
    return probs, 1. / num_examples * total_loss    
...
def showValidationResults(probs):
    ...
    for i in range(len(pred_y)):
        ...
        if (pred_y[i] == actual_y[i]):
            ...
            print("Predicted: " + pred_y[i] + " | Actual: " + actual_y[i])
